...
public class AirLineApp {
	...
	private static void print_header() {
		...
		System.out.println(
				cc.RED_BACKGROUND_BRIGHT + cc.BLACK_BOLD + "       Airline Reservation System       " + cc.RESET);
		System.out.println(cc.GREY_BACKGROUND + "<><><><><><><><><><><><><><><><><><><><>" + cc.RESET);
		System.out.print("\n");
	}
				...
				if (ProjectDB.person_list.size() == 0) {
					...
					if (ProjectDB.scheduled_flight_list.size() == 0) {
						...
						if (scf.capacity == Passenger.getSCFlightPassengersCount(scf.flight_number)
								|| ProjectDB.passenger_list.size() == 0) {
							...
							if (prevLen != afterLen) {
								System.out.println("Reservation completed : " + p.PersnDet.sName + " (" + scf.from
										+ " -> " + scf.to + ")\n");
							}
						}
						passengers_menu();
